...
interface MusicDao {
    ...
    // --- Song Queries ---
    // Include cloud songs (negative IDs) regardless of directory filter.
    @Query("SELECT " + SONG_LIST_PROJECTION + """
        FROM songs
        WHERE (:applyDirectoryFilter = 0 OR source_type != 0 OR parent_directory_path IN (:allowedParentDirs))
        ORDER BY title ASC
    """)
    fun getSongs(
    ...
    @Query("SELECT " + SONG_LIST_PROJECTION + """
        FROM songs
        WHERE id IN (:songIds)
        AND (:applyDirectoryFilter = 0 OR source_type != 0 OR parent_directory_path IN (:allowedParentDirs))
    """)
    fun getSongsByIds(
        songIds: List<Long>,
        allowedParentDirs: List<String>,
    ...
    @Query("""
        ...
        ORDER BY
            ...
            songs.id ASC
    """)
    suspend fun getFavoriteSongsSortedList(
        allowedParentDirs: List<String>,
        applyDirectoryFilter: Boolean,
        sortOrder: String,
        filterMode: Int
